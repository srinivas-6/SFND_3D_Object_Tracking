...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
  ...
  {
   const auto &currpoint = kptsCurr[match.trainIdx].pt;
  ...
  // calculate mean distances of all matches present in bounding box
  for(auto &it:boundingBox.kptMatches)
  {
    cv::KeyPoint &curr = kptsCurr.at(it.trainIdx);
    cv::KeyPoint &prev = kptsPrev.at(it.queryIdx);
    dist = cv::norm(curr.pt-prev.pt);
    sum+=dist;
  }
                     ...
                     std::vector<LidarPoint> &lidarPointsCurr, double frameRate, double &TTC)
{
    // ...
  double dT = 1/frameRate;
  double lanewidth = 4.0; // assuming width of the ego lane from lectures
